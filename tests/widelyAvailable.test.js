...
Object.keys(obj).forEach(key => {
  ...
  describe("lastIndexOfToEndsWith", () => {
    ...
    describe("promiseToAsyncAwait", () => {
      test("promise.then().catch() pattern", () => {
        const result = transform(`
function doWork() {
  promise
    .then(result => {
...
`)
        ...
        assert.match(result.code, /const data = await fetchData/)
      })

      test("promise chain with function expression", () => {
        const result = transform(`
const fn = function() {
  getUser()
    .then(user => {
...
`)
        ...
        assert(result.modified, "transform promise chain in function expression")
        assert.match(result.code, /async function fn/)
        assert.match(result.code, /const user = await getUser/)
      })
