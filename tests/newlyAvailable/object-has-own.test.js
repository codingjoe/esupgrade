...
suite("newly-available", () => {
  describe("Object.hasOwn", () => {
    ...
    test("transform object literal hasOwnProperty.call", () => {
      const result = transform(`({}).hasOwnProperty.call(obj, prop)`, "newly-available")
    ...
    test("skip in widely-available baseline", () => {
      const result = transform(
        `Object.prototype.hasOwnProperty.call(obj, prop)`,
        "widely-available",
      )

      assert(!result.modified, "skip Object.hasOwn outside newly-available baseline")
    })
    ...
    test("skip unsupported receiver", () => {
      const result = transform(`dict.hasOwnProperty.call(obj, prop)`, "newly-available")
