 ...
 * These functions provide reusable checks and operations for AST manipulation.
 */

/**
 * String methods that return iterable values.
 */
 ...
 */
export function containsIdentifier(j, node, varName) {
  if (!node) {
    return false
  }
    ...
    return node.properties.some(
      (prop) =>
        ((j.Property.check(prop) || j.ObjectProperty.check(prop)) &&
          containsIdentifier(j, prop.value, varName)) ||
        (j.RestElement.check(prop) && containsIdentifier(j, prop.argument, varName)),
    )
  }
  if (j.ArrayPattern.check(node)) {
    return node.elements.some((element) => containsIdentifier(j, element, varName))
  }
  if (j.AssignmentPattern.check(node)) {
    return containsIdentifier(j, node.left, varName)
  }
  // RestElement is the only remaining valid pattern type
  return j.RestElement.check(node) && containsIdentifier(j, node.argument, varName)
}
 ...
 */
export function* extractIdentifiers(j, pattern) {
  if (!pattern) return
  ...
  if (j.Identifier.check(pattern)) {
    ...
    for (const prop of pattern.properties) {
      if (j.Property.check(prop) || j.ObjectProperty.check(prop)) {
        yield* extractIdentifiers(j, prop.value)
      } else if (j.RestElement.check(prop)) {
        yield* extractIdentifiers(j, prop.argument)
      }
    }
  } else if (j.ArrayPattern.check(pattern)) {
    for (const element of pattern.elements) {
      yield* extractIdentifiers(j, element)
    }
  } else if (j.AssignmentPattern.check(pattern)) {
    yield* extractIdentifiers(j, pattern.left)
  } else if (j.RestElement.check(pattern)) {
    yield* extractIdentifiers(j, pattern.argument)
  }
}
 ...
 */
export function areEquivalent(j, node1, node2) {
  ...
  if (j.MemberExpression.check(node1) && j.MemberExpression.check(node2)) {
    return (
      areEquivalent(j, node1.object, node2.object) &&
      areEquivalent(j, node1.property, node2.property) &&
  ...
  if (j.CallExpression.check(node1) && j.CallExpression.check(node2)) {
    ...
    if (!areEquivalent(j, node1.callee, node2.callee)) {
      return false
    }
    ...
    for (let i = 0; i < node1.arguments.length; i++) {
      if (!areEquivalent(j, node1.arguments[i], node2.arguments[i])) {
        return false
      }
    }
    return true
 ...
 */
export function isVerifiableIterable(j, node) {
  // Array literal: [1, 2, 3]
 ...
 */
export function isAssignmentShadowed(j, varName, declarationPath, usagePath) {
    ...
    if (
      ...
      if (current.node.params) {
        for (const param of current.node.params) {
          if (containsIdentifier(j, param, varName)) {
            return true
          }
        }
      }
      ...
      if (functionBody) {
        ...
        const hasLocalDecl = j(functionBody)
          ...
          .some((declPath) => {
            ...
            return containsIdentifier(j, declPath.node.id, varName)
          })
 ...
 */
export function isReassigned(j, root, varName, declarationPath) {
  ...
  root.find(j.AssignmentExpression).forEach((assignPath) => {
    if (!containsIdentifier(j, assignPath.node.left, varName)) {
      return
    }
