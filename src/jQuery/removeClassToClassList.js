...
/**
 * Transform $(el).removeClass('a b') to el.classList.remove('a', 'b').
 *
 * @param {import('jscodeshift').Collection} root
 * @returns {boolean} True if modified.
 */
export function removeClassToClassList(root) {
  let modified = false
  root
    ...
    .forEach((path) => {
      ...
      if (!member.property || member.property.name !== "removeClass") return
