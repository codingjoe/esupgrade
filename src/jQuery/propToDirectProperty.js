...
/**
 * Get the root identifier name from a member expression.
 *
 * @param {import('jscodeshift').MemberExpression} memberExpr - The member expression.
 * @returns {string|null} The root identifier name or null.
 */
...
/**
 * Check if node is a document.querySelector or document.getElementById call.
 *
 * @param {import('jscodeshift').Node} node - The node to check.
 * @returns {boolean} True if the node is a single element call.
 */
function isDocumentSingleElementCall(node) {
  ...
  if (!["querySelector", "getElementById"].includes(callee.property.name)) return false
...
/**
 * Resolve target to a single element expression.
 *
 * @param {import('jscodeshift').Collection} root - The root collection.
 * @param {import('jscodeshift').Node} node - The node to resolve.
 * @returns {import('jscodeshift').Node|null} The resolved node or null.
 */
