...
/**
 * Replace $(el).click(handler) with el.addEventListener('click', handler).
 *
 * @param {import('jscodeshift').Collection} root - The root AST collection.
 * @returns {boolean} True if code was modified.
 */
export function clickToAddEventListener(root) {
  let modified = false
  root
    ...
    .forEach((path) => {
      ...
      if (!member.property || member.property.name !== "click") return
      ...
      const call = j.callExpression(
        j.memberExpression(target, j.identifier("addEventListener")),
        [j.literal("click"), handler],
      )
